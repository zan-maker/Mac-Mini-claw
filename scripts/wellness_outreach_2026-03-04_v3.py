...
Sends initial outreach to high-priority leads via AgentMail API
"""
...
# AgentMail Configuration - Using verified key from config
...
REPLY_TO_EMAIL = "sam@impactquadrant.info"
REPLY_TO_NAME = "Sam Desigan"
...
def send_email(to_email, subject, text_body, html_body):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {
                "success": False,
                "error": f"HTTP {response.status_code}: {response.text[:5]}",
                "status": "failed"
            }
