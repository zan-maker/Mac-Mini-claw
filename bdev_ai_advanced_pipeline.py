...
"""
    ...
    def run_pipeline(self, investor_count: int = 50):
        ...
        try:
            ...
            df, csv_path = self.generate_ai_messages(investor_count)
            
            # Step 2: Send via AgentMail
            send_results = self.send_messages(df)
            
            # Step 3: Create reports
            report_path, log_path, results_path = self.create_detailed_report(df, send_results, csv_path)
            ...
            print(f"📊 Results:")
            print(f"   • Total Processed: {send_results['sent'] + send_results['failed'] + send_results['skipped']}")
            print(f"   • ✅ Sent: {send_results['sent']}")
            print(f"   • ❌ Failed: {send_results['failed']}")
            print(f"   • ⏭️ Skipped: {send_results['sk