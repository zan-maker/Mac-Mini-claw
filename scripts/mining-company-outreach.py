#!/usr/bin/env python3
"""
Mining Company Outreach Script
...
CC_EMAIL = "sam@cubiczan.com"  # User requested CC
...
Please reach out to Sam Desigan (sam@cubiczan.com) for further follow up.
"""
    
    return body

def send_email_via_agentmail(to_email, company_name, subject, body):
...
def main():
    ...
    for company in COMPANIES:
        ...
        send_result = send_email_via_agentmail(
            emails[0],
            company['name'],
            subject,
            email_body
    ...
    # Save results
    output_file = f"/Users/cubiczan/.openclaw/workspace/mining-outreach-results-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
