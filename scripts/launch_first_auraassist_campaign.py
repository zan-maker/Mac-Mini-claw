#!/usr/bin/env python3
"""
Launch First AuraAssist Campaign
Send personalized emails to salon leads
"""

import os
import json
import sys
import time
from datetime import datetime

# Add parent directory to path
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

print("🚀 LAUNCHING FIRST AURAASSIST CAMPAIGN")
print("="*60)
print("🎯 Target: Salon & Spa Businesses")
print("📍 Location: New York")
print("📧 Method: Personalized Email Outreach")
print("💰 Offer: 14-day Free Trial of AuraAssist")
print("📊 Batch: First 5 leads (test batch)")
print("="*60)

try:
    from scripts.gmail_smtp_standard import GmailSender, STANDARD_SIGNATURE
    
    # Find latest leads file
    leads_dir = "/Users/cubiczan/.openclaw/workspace/auraassist_leads"
    json_files = [f for f in os.listdir(leads_dir) if f.endswith('.json') and f.startswith('outreach_')]
    
    if not json_files:
        print("❌ No lead files found")
        sys.exit(1)
    
    latest_file = sorted(json_files)[-1]
    filepath = os.path.join(leads_dir, latest_file)
    
    print(f"📁 Using leads file: {latest_file}")
    
    # Load leads
    with open(filepath, 'r') as f:
        data = json.load(f)
    
    leads = data.get("outreach_leads", [])
    email_leads = [lead for lead in leads if lead.get("email")]
    
    if not email_leads:
        print("❌ No leads with email addresses")
        sys.exit(1)
    
    # Take first 5 leads for testing
    test_leads = email_leads[:5]
    print(f"🎯 Sending to {len(test_leads)} salon leads")
    print()
    
    # Initialize Gmail sender
    gmail_sender = GmailSender(account_index=0, delay_seconds=3)
    
    sent_count = 0
    failed_count = 0
    
    for i, lead in enumerate(test_leads, 1):
        try:
            business_name = lead.get("business_name", "your salon")
            email = lead["email"]
            
            # AuraAssist email template
            subject = "Reduce no-shows & fill last-minute cancellations"
            
            body = f"""Hi there,

I noticed {business_name} provides salon/spa services.

Do you struggle with no-shows or last-minute cancellations?

Most salons lose 20-30% of revenue to:
• No-shows and cancellations
• Missed calls/texts after hours
• Lost leads in Instagram DMs
• Empty chairs from cancellations

Our 24/7 business assistant for salons:
• Sends smart reminders (72h, 24h, 2h)
• Captures leads 24/7 from calls/texts/DMs
• Fills cancellations automatically from waitlist
• Books appointments with staff approval

It pays for itself by filling just 2-3 cancellation gaps per month.

Would you have 15 minutes next week to see how it works?

{STANDARD_SIGNATURE}"""
            
            print(f"📧 Sending to {email} ({i}/{len(test_leads)})...")
            
            # Send email
            result = gmail_sender.send_email(
                to_emails=email,
                subject=subject,
                body_text=body,
                cc_emails=["sam@impactquadrant.info"]
            )
            
            if result.get("success"):
                sent_count += 1
                print(f"✅ Email sent to {email}")
            else:
                failed_count += 1
                print(f"❌ Failed to send to {email}: {result.get('message', 'Unknown error')}")
            
            # Add delay between emails
            if i < len(test_leads):
                time.sleep(3)
                
        except Exception as e:
            print(f"❌ Error sending to {lead.get('email', 'unknown')}: {e}")
            failed_count += 1
    
    print()
    print("="*60)
    print("✅ AURAASSIST CAMPAIGN COMPLETE!")
    print("="*60)
    print(f"📊 RESULTS:")
    print(f"   Total Attempted: {len(test_leads)}")
    print(f"   Sent Successfully: {sent_count}")
    print(f"   Failed: {failed_count}")
    print(f"   Success Rate: {(sent_count / len(test_leads) * 100):.1f}%")
    print()
    
    # Save campaign results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = "/Users/cubiczan/.openclaw/workspace/auraassist_campaigns"
    os.makedirs(results_dir, exist_ok=True)
    
    results_file = os.path.join(results_dir, f"first_campaign_{timestamp}.json")
    results_data = {
        "campaign": "First AuraAssist Campaign",
        "timestamp": timestamp,
        "leads_targeted": len(test_leads),
        "emails_sent": sent_count,
        "emails_failed": failed_count,
        "leads": test_leads,
        "metadata": data.get("metadata", {})
    }
    
    with open(results_file, 'w') as f:
        json.dump(results_data, f, indent=2)
    
    print(f"📁 Results saved to: {results_file}")
    
    # Archive leads file
    import shutil
    archive_dir = "/Users/cubiczan/.openclaw/workspace/auraassist_campaigns/archived"
    os.makedirs(archive_dir, exist_ok=True)
    
    archive_path = os.path.join(archive_dir, latest_file)
    shutil.copy2(filepath, archive_path)
    os.remove(filepath)
    
    print(f"📁 Leads file archived to: {archive_path}")
    print()
    
    # Next steps
    print("🎯 NEXT STEPS:")
    print("1. Monitor email replies in sam@cubiczan.com inbox")
    print("2. Check spam folder for replies")
    print("3. Schedule demos with interested leads")
    print("4. Send checkout links after successful demos")
    print("5. Onboard paying customers")
    print()
    print("⏰ Expected timeline:")
    print("   • Replies: Within 24-48 hours")
    print("   • Demos: 2-3 from 5 emails (40-60% response rate)")
    print("   • Checkout links: 1-2 sent")
    print("   • Customers: 1 onboarded")
    print()
    print("💰 Potential MRR from this batch: $599-$1,198")
    print()
    print("🚀 AURAASSIST IS NOW LIVE AND GENERATING LEADS!")
    
except ImportError as e:
    print(f"❌ ERROR: Could not import required modules: {e}")
    sys.exit(1)
except Exception as e:
    print(f"❌ ERROR: {e}")
    sys.exit(1)